...
static void zend_generic_scope_pop(void) /* {{{ */
{
...
{
	if (!caller_op_array || !caller_op_array->generic_types || !caller_op_array->generic_types->turbofish_args || args_id == 0) {
		return NULL;
	}

	return zend_hash_index_find_ptr(caller_op_array->generic_types->turbofish_args, args_id);
}

static void zend_check_generic_argument_bounds(
...
ZEND_API void zend_check_generic_static_class_arguments(const zend_class_entry *ce, uint32_t arity, const zend_type *args_box)
{
	...
	if (arity > total) {
		zend_throw_error(zend_ce_argument_count_error,
			"Too many generic type arguments to class %s, %u passed and %s %u expected",
			ZSTR_VAL(ce->name), arity,
			required == total ? "exactly" : "at most", total);
		return;
	}
	if (arity < required) {
		zend_throw_error(zend_ce_argument_count_error,
			"Too few generic type arguments to class %s, %u passed and %s %u expected",
			ZSTR_VAL(ce->name), arity,
			required == total ? "exactly" : "at least", required);
		return;
	}
	...
	if (args_box) {
		zend_check_generic_argument_bounds(ce->generic_parameters, args_box, arity, "class", ZSTR_VAL(ce->name));
	}
}

#define ZEND_VERIFY_ARITY_KIND_CALL          0
#define ZEND_VERIFY_ARITY_KIND_NEW           1
#define ZEND_VERIFY_ARITY_KIND_STATIC_CLASS  2

static zend_type zend_compile_turbofish_args_type(zend_ast *turbofish_ast)
{
	zend_ast_list *list = zend_ast_get_list(turbofish_ast);
	zend_type_named_with_args *payload = emalloc(ZEND_TYPE_NAMED_WITH_ARGS_SIZE(list->children));
	payload->name = NULL;
	payload->name_attr = 0;
	payload->count = list->children;
	for (uint32_t i = 0; i < list->children; i++) {
		payload->args[i] = zend_compile_pre_erasure_typename(list->child[i]);
	}
...
static void zend_emit_verify_generic_arguments(zend_ast *turbofish_ast, uint8_t kind, const znode *new_result)
{
	...
	zend_type args_type = zend_compile_turbofish_args_type(turbofish_ast);
	...
	opline->opcode = ZEND_VERIFY_GENERIC_ARGUMENTS;
	opline->op2_type = IS_UNUSED;
	opline->op2.num = arity;
	opline->extended_value = args_id;
	if (kind == ZEND_VERIFY_ARITY_KIND_NEW) {
		...
		opline->op1.num = (kind == ZEND_VERIFY_ARITY_KIND_STATIC_CLASS) ? 1 : 0;
	}
	...
	for (uint32_t i = 0; i < list->children; i++) {
		...
		for (uint32_t j = 0; j < i; j++) {
			if (zend_string_equals(params->parameters[j].name, name)) {
				...
				zend_error_noreturn(E_COMPILE_ERROR,
					"Cannot redeclare type parameter %s", ZSTR_VAL(dup));
			}
		}
		...
		if (param) {
			...
			if (code != 0) {
				...
				if (zend_get_class_fetch_type(name) != ZEND_FETCH_CLASS_DEFAULT) {
					resolved = zend_string_copy(name);
				} else {
					resolved = zend_resolve_class_name(name, ast->attr);
				}
				result = (zend_type) ZEND_TYPE_INIT_CLASS(resolved, 0, 0);
			}
		}
	} else {
		ZEND_UNREACHABLE();
...
static zend_generic_parameter *zend_generic_lookup_name(zend_ast *ast) /* {{{ */
{
	...
	return zend_generic_lookup(Z_STR_P(zv));
}
/* }}} */
...
zend_string *zend_type_to_string_resolved(const zend_type type, const zend_class_entry *scope) {
	...
	if (ZEND_TYPE_IS_INTERSECTION(type)) {
		...
		smart_str buf = {0};
		if (nwa->name) {
			zend_string *resolved = resolve_class_name(nwa->name, scope);
			smart_str_append(&buf, resolved);
			zend_string_release(resolved);
		}

		smart_str_appendc(&buf, '<');
		for (uint32_t i = 0; i < nwa->count; i++) {
			if (i > 0) smart_str_appends(&buf, ", ");
			zend_string *piece = zend_type_to_string_resolved(nwa->args[i], scope);
			if (piece) {
...
static bool zend_compile_call_common(znode *result, zend_ast *args_ast, const zend_function *fbc, uint32_t lineno, uint32_t type, zend_ast *turbofish_ast, uint8_t verify_kind, const znode *new_result, zend_ast *class_turbofish_ast) /* {{{ */
{
	zend_op *opline;
	uint32_t opnum_init = get_next_op_number() - 1;

	zend_emit_verify_generic_arguments(turbofish_ast, verify_kind, new_result);
	if (class_turbofish_ast) {
		zend_emit_verify_generic_arguments(class_turbofish_ast, ZEND_VERIFY_ARITY_KIND_STATIC_CLASS, NULL);
	}
...
static void zend_compile_dynamic_call(znode *result, znode *name_node, zend_ast *args_ast, zend_ast *turbofish_ast, uint32_t lineno, uint32_t type) /* {{{ */
{
	...
	zend_compile_call_common(result, args_ast, NULL, lineno, type, turbofish_ast, ZEND_VERIFY_ARITY_KIND_CALL, NULL, NULL);
}
/* }}} */
...
static void zend_compile_assert(znode *result, zend_ast_list *args, zend_string *name, const zend_function *fbc, uint32_t lineno, uint32_t type) /* {{{ */
{
	if (EG(assertions) >= 0) {
		...
		zend_compile_call_common(result, (zend_ast*)args, fbc, lineno, type, NULL, ZEND_VERIFY_ARITY_KIND_CALL, NULL, NULL);
...
static zend_result zend_compile_func_array_map(znode *result, zend_ast_list *args, zend_string *lcname, uint32_t lineno) /* {{{ */
{
	...
	switch (callback->kind) {
		case ZEND_AST_CALL:
			...
			break;
		case ZEND_AST_STATIC_CALL:
			zend_compile_expr(&call_result, zend_ast_create(ZEND_AST_STATIC_CALL, callback->child[0], callback->child[1], call_args, NULL, NULL));
			break;
	}
...
static bool zend_compile_parent_property_hook_call(znode *result, const zend_ast *ast, uint32_t type)
{
	...
	zend_compile_call_common(result, args_ast, NULL, zend_ast_get_lineno(method_ast), BP_VAR_R, NULL, ZEND_VERIFY_ARITY_KIND_CALL, NULL, NULL);
...
static void zend_compile_call(znode *result, const zend_ast *ast, uint32_t type) /* {{{ */
{
...

	{
		...
		zend_compile_call_common(result, args_ast, fbc, ast->lineno, type, turbofish_ast, ZEND_VERIFY_ARITY_KIND_CALL, NULL, NULL);
	}
}
/* }}} */
...
static void zend_compile_method_call(znode *result, zend_ast *ast, uint32_t type) /* {{{ */
{
	...
	if (zend_compile_call_common(result, args_ast, fbc, zend_ast_get_lineno(method_ast), type, turbofish_ast, ZEND_VERIFY_ARITY_KIND_CALL, NULL, NULL)) {
...
static void zend_compile_static_call(znode *result, zend_ast *ast, uint32_t type) /* {{{ */
{
	...
	zend_ast *args_ast = ast->child[2];
	zend_ast *turbofish_ast = ast->child[3];
	zend_ast *class_turbofish_ast = ast->child[4];
	...
	zend_compile_call_common(result, args_ast, fbc, zend_ast_get_lineno(method_ast), type, turbofish_ast, ZEND_VERIFY_ARITY_KIND_CALL, NULL, class_turbofish_ast);
}
/* }}} */
...
static void zend_compile_new(znode *result, zend_ast *ast) /* {{{ */
{
	...
	zend_compile_call_common(&ctor_result, args_ast, fbc, ast->lineno, BP_VAR_R, turbofish_ast, ZEND_VERIFY_ARITY_KIND_NEW, result, NULL);
...
static void zend_compile_pipe(znode *result, zend_ast *ast, uint32_t type)
{
	...
	/* Turn $foo |> bar::baz(...) into bar::baz($foo). */
		...
		fcall_ast = zend_ast_create(ZEND_AST_STATIC_CALL,
			callable_ast->child[0], callable_ast->child[1], arg_list_ast, NULL, NULL);
	/* Turn $foo |> $bar->baz(...) into $bar->baz($foo). */
...
static void zend_compile_attributes(
	...
	for (g = 0; g < list->children; g++) {
		...
		for (i = 0; i < group->children; i++) {
			...
			if (el->child[2]) {
				zend_type compiled = zend_compile_turbofish_args_type(el->child[2]);
				attr->generic_args = emalloc(sizeof(zend_type));
				*attr->generic_args = compiled;
			}
			zend_string_release(name);
...
static zend_op_array *zend_compile_func_decl_ex(
	...
	zend_ast *generic_params_ast = decl->child[5];
